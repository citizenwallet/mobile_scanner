...
public class MobileScannerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterTexture,
    AVCaptureVideoDataOutputSampleBufferDelegate
{
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        #if os(iOS)
            let orientationEvent = FlutterEventChannel(
                name:
                    "dev.steenbakker.mobile_scanner/scanner/deviceOrientation",
                binaryMessenger: messenger)
            orientationEvent.setStreamHandler(
                DeviceOrientationStreamHandler(onOrientationChanged: instance.setDeviceOrientation))
        #endif
    }
    ...
    public func captureOutput(
        ...
        if (detectionSpeed == DetectionSpeed.normal
            || detectionSpeed == DetectionSpeed.noDuplicates) && eligibleForScan
            || detectionSpeed == DetectionSpeed.unrestricted
        {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                VTCreateCGImageFromCVPixelBuffer(
                    self!.latestBuffer, options: nil, imageOut: &cgImage)
                ...
                do {
                    let barcodeRequest: VNDetectBarcodesRequest = VNDetectBarcodesRequest(
                        completionHandler: { [weak self] (request, error) in
                            ...
                            guard
                                let results: [VNBarcodeObservation] = request.results
                            ...
                            DispatchQueue.main.async {
                                ...
                                guard let image = cgImage else {
                                    self?.sink?([
                                        "name": "barcode",
                                        "data": barcodes.map({
                                            $0.toMap(imageWidth: 0, imageHeight: 0, scanWindow: nil)
                                        }),
                                    ])
                                    return
                                }
                                ...
                                let imageData: [String: Any?] = [
                                    "bytes": MobileScannerPlugin.returnImage
                                        ? FlutterStandardTypedData(
                                            bytes: image.jpegData(compressionQuality: 0.8)!) : nil,
    ...
    func start(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        if device != nil || captureSession != nil {
            result(
                FlutterError(
                    code: MobileScannerErrorCodes.ALREADY_STARTED_ERROR,
        ...
        if device == nil {
            result(
                FlutterError(
                    code: MobileScannerErrorCodes.NO_CAMERA_ERROR,
        ...
        device.addObserver(
            self, forKeyPath: #keyPath(AVCaptureDevice.torchMode), options: .new, context: nil)
        #if os(iOS)
            device.addObserver(
                self, forKeyPath: #keyPath(AVCaptureDevice.videoZoomFactor),
            ...
            if #available(iOS 13.0, *) {
                for (index, actualDevice) in device.constituentDevices.enumerated() {
                    if actualDevice.deviceType != .builtInUltraWideCamera {
                        if index > 0
                            && index <= device.virtualDeviceSwitchOverVideoZoomFactors.count
                        {
                            standardZoomFactor = CGFloat(
            ...
            if !(captureSession!.canAddInput(input)) {
                result(
                    FlutterError(
                        ...
                        message: MobileScannerErrorCodes
                            .CAMERA_ERROR_CAPTURE_SESSION_INPUT_OCCUPIED_MESSAGE,
                        details: nil))
                return
            }
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            DispatchQueue.main.async {
                ...
                if torch {
                    ...
                    let size = [
                        "width": Double(dimensions.height), "height": Double(dimensions.width),
                    ]
                #else
                    let size = [
    ...
    private func setScale(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        do {
            ...
            result(
                FlutterError(
                    code: MobileScannerErrorCodes.SET_SCALE_WHEN_STOPPED_ERROR,
    ...
    func setScaleInternal(_ scale: CGFloat) throws {
        ...
        func updateOrientation(orientation: UIDeviceOrientation) {
            if let videoOutput = captureSession!.outputs.compactMap({
        ...
        case AVCaptureDevice.TorchMode.auto:
            if #available(macOS 10.15, *) {
                newTorchMode =
                    ...
                    ? AVCaptureDevice.TorchMode.off : AVCaptureDevice.TorchMode.on
            }
            break
    ...
    func analyzeImage(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        // See https://forums.developer.apple.com/forums/thread/696714
            ...
            result(
                FlutterError(
                    ...
                    message: MobileScannerErrorCodes
                        .ANALYZE_IMAGE_IOS_SIMULATOR_NOT_SUPPORTED_ERROR_MESSAGE,
                    details: nil
                ))
            return
        #endif
        ...
        let imageRequestHandler = VNImageRequestHandler(
            ciImage: ciImage, orientation: CGImagePropertyOrientation.up, options: [:])
        ...
        do {
            let barcodeRequest: VNDetectBarcodesRequest = VNDetectBarcodesRequest(
                completionHandler: { [] (request, error) in
                    ...
                    guard
                        let barcodes: [VNBarcodeObservation] = request.results
                            as? [VNBarcodeObservation]
                    else {
                        DispatchQueue.main.async {
                            result([
...
extension CGImage {
    public func jpegData(compressionQuality: CGFloat) -> Data? {
        ...
        guard let destination = CGImageDestinationCreateWithData(mutableData!, formatHint, 1, nil)
        else {
            return nil
        }
